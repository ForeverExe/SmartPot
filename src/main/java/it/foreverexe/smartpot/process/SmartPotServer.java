...
public class SmartPotServer {
    ...
    static String infoTopic = MqttConfigurationParameters.MQTT_BASIC_TOPIC + "/+"
            + MqttConfigurationParameters.MQTT_INFO_TOPIC;
    static String telTopic = MqttConfigurationParameters.MQTT_BASIC_TOPIC + "/+"
            + MqttConfigurationParameters.MQTT_TELEMETRY_BASIC_TOPIC;
    static String settingTopic = MqttConfigurationParameters.MQTT_BASIC_TOPIC + "/+"
            + MqttConfigurationParameters.MQTT_SETTINGS_TOPIC;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Subscribing to topic:" + settingTopic);
            mqttClient.subscribe(settingTopic, 1, new IMqttMessageListener() {
                @Override
                public void messageArrived(String topic, MqttMessage message) throws Exception {
                    ...
                    System.out.println("In oggetto: " + PotsList.get(uuid).getSettings());
                }
            });

            //solo iniziale per sincronizzare, dopo non serve piú.
            mqttClient.unsubscribe(settingTopic);

            // Iscriviti a tutte le telemetrie dei dispositivi, poi si occupa di segnare nel
            // dispositivo corretto i vari dati
            ...
            while (running) {
                ...
                switch (choice) {
                    case 0:
                        ...
                        SmartPotSettings settings = PotsList.get(deviceKey).getSettings();
                        ...
                        String topic = MqttConfigurationParameters.MQTT_BASIC_TOPIC + "/" + PotsList.get(deviceKey).getUuid()
                                + MqttConfigurationParameters.MQTT_SETTINGS_TOPIC;
    ...
    /**
     * Ask the user to choose a device by index. Returns the device key (name) or
     * null on cancel/invalid.
     */
